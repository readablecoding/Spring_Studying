...
public class NoticeController {
	...
	private final int countPerPage = 3; //3개씩 보여주기
	private final int pagePerGroup = 5; //5개를 하나의 그룹으로 함
	
	
	@Autowired
	private noticeDAO dao;
	
	//Parameter -Current Page 값(default = 1)
	...
	public String noticeList(Model model,
		...
		//PageNavigator 객체를 만든다. countPerPage, pagePerGroup는 상수로 지정한 것, page는 파라미터로 받은 것, total는 dao를 통해 받아옴
		PageNavigator navi = new PageNavigator(countPerPage, pagePerGroup, page, total);
		
		
		//시작 위치, 페이지당 글 개수 -> 알맞은 글을 가져올 수 있게 한다. 검색어가 포함된 1페이지를 그린다.
		ArrayList<Notice> list = dao.noticeSelectList(searchText, navi.getStartRecord(), navi.getCountPerPage());
		logger.info("리스트로 값 넘김 {}", list);
		
		model.addAttribute("list", list);
		model.addAttribute("navi", navi); //PageNavigator 객체의 데이터를 가지고 jsp로 돌아간다.
		model.addAttribute("searchText", searchText);
		
		logger.info("navi의 값: {}", navi);
		logger.info("searchText의 값: {}", searchText);
